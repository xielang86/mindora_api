...
class XunFeiSRT(BaseSRT):
    ...
    def on_message(self, ws, message):
        ...
        try:
            ...
            if code != 0:
                ...
                L.warning(f"讯飞语音听写 sid:{sid} 调用出错: {errMsg} 错误码:{code}")
                ...
                for item in ws_data:
                    for w in item.get("cw", []):
                        self.result += w.get("w", "")
                L.debug(f"讯飞语音听写 sid:{sid} 识别结果: ### {self.result} ###")

                # 如果是最后一条消息，则设置完整结果
                if status == self.STATUS_LAST_FRAME:
                    ws.close()
        except Exception as e:
            L.error(f"讯飞语音听写处理响应时出错: {e} trace:{traceback.format_exc()}")
            ws.close()
    ...
    def on_error(self, ws, error):
        ...
        处理WebSocket错误
        """
        L.error(f"讯飞语音听写WebSocket错误: {error}")
    ...
    def on_open(self, ws, audio_path: str):
        """
        ...
        def run():
            ...
            L.debug("讯飞语音听写WebSocket连接已打开，开始发送音频数据...")
                ...
                with open(audio_path, "rb") as fp:
                    ...
                    while True:
                        ...
                        if status == self.STATUS_FIRST_FRAME:
                            ...
                            frame_count += 1
                            L.debug(f"讯飞语音听写发送第{frame_count}帧 (第一帧)")
                            ...
                            ws.send(json.dumps(data))
                            frame_count += 1
                            L.debug(f"讯飞语音听写发送第{frame_count}帧 (最后一帧)")
                            time.sleep(1)  # 等待处理
                            break

                        time.sleep(intervel)
                    L.debug("讯飞语音听写音频数据发送完毕。")
            except Exception as e:
                L.debug(f"讯飞语音听写发送音频时出错: {e}")
                ws.close()
                raise

        self.executor.submit(run)

    async def recognize(self, audio_path: str) -> SRTRsp:
        """
        ...
        ws_app.on_open = lambda ws: self.on_open(ws, audio_path)
