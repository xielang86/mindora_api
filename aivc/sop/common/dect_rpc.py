...
from aivc.sop.common.common import ApiResponse, SleepResult, PoseInfo, VoiceSequence, Actions, AbNormalState
import zerorpc
from aivc.config.config import L,settings
import traceback
import json
from aivc.sop.sleep.config import SleepStateType, states_dict
from aivc.sop.common.voice import voice_manager
import base64
import time
from aivc.sop.common.common import EyeStatus, BodyPoseType, SleepStatus, PersonStatus
...
async def get_actions(state:SleepStateType) -> Actions:
    actions = states_dict.get(state)
    
    # 处理异常状态，如玩手机或坐起
    if state and AbNormalState.is_abnormal(state) and actions and actions.voice and actions.voice.voices:
        # 从语音列表中随机选择一个
...
async def get_state_voices_duration(state: SleepStateType) -> float:
    total_duration = 0.0
    actions = states_dict.get(state)
...
def determine_state_type(sleep_status: str, pose: PoseInfo, recent_action: str) -> tuple[SleepStateType, str, str]:
    state_type = None
    eye_status = None
    lie_status = None
    ...
    if sleep_status ==SleepStatus.Awake.value:
        if pose is not None:
            ...
            # SleepStateType.PREPARE
            if (body_pose in LIE_POSES and
                left_hand_pose != "LiftOn" and 
                right_hand_pose != "LiftOn"):
                state_type = SleepStateType.PREPARE
            
            # SleepStateType.SITTING_UP
            if body_pose == "SitDown":
                state_type = SleepStateType.SITTING_UP 
            
            # SleepStateType.USING_PHONE
            if (body_pose in LIE_POSES + ["SitDown"] and
                (left_hand_pose == "LiftOn" or 
                right_hand_pose == "LiftOn")):
                state_type = SleepStateType.USING_PHONE 
                            
            # SleepStateType.LEAVING
            if body_pose == "Stand":
                state_type = SleepStateType.LEAVING 
        else:
            state_type = SleepStateType.LEAVING 
...
async def llm_response_to_state(response: dict) -> tuple[SleepStateType, str, str]:
    state = None
    eye_status = None
    lie_status = None
    try:
        ...
        if person_status and person_status.sleepSignals:
            ...
            if handActivity == "active_device_use":
                state = SleepStateType.USING_PHONE
