...
from aivc.sop.sleep.config import SleepStateType 
from typing import Optional
import time
from aivc.sop.common.common import EyeStatus, BodyPoseType, SceneExecStatus,AbNormalState
...
class StateManager:
    ...
    async def update_state_by_exec_status(
            ...
            scene_exec_status: SceneExecStatus) -> Optional[SleepStateType]:
        async with self._lock:
            rsp_state = await self.get_current_state(conversation_id)
            if scene_exec_status and scene_exec_status.scene_seq < (SleepStateType.SLEEP_READY.order - 1) and scene_exec_status.status == "COMPLETED":
                rsp_state = SleepStateType.get_next_state(scene_exec_status.scene_seq)
    ...
    async def get_current_state(self, conversation_id: str) -> Optional[SleepStateType]:
        return self._state_cache.get(conversation_id, SleepStateType.PREPARE)
    
    async def set_state(self, conversation_id: str, state: SleepStateType) -> None:
        if state is not None and not AbNormalState.is_abnormal(state):
...
class SleepMonitor:
    ...
    async def check_sleep_condition(self, 
            ...
            eye_status: str, 
            lie_status: str, 
            current_state: SleepStateType) -> None:
        abnormal_states = [SleepStateType.USING_PHONE, SleepStateType.SITTING_UP, SleepStateType.LEAVING]
