...
from aivc.common.sleep_alert import AlertLevel, SleepAlert
...
class AlertStateManager:
    ...
    async def update_state(self, conversation_id: str, state: StateType) -> Tuple[Optional[AlertLevel], Optional[Actions]]:
        """
        更新状态并检查是否触发告警
        ...
        async with self._lock:
            ...
            # 将状态历史转换为简化表示
            L.debug(f"AlertStateManager 会话 {conversation_id} 记录状态: {state.name_cn}")
    ...
    def _check_state_pattern(self, conversation_id: str) -> Optional[AlertLevel]:
        ...
        for state in history:
            if state == StateType.USING_PHONE:
                pattern.append("Y")
            elif state == StateType.SITTING_UP:
                pattern.append("J")
            elif state == StateType.LEAVING:
                pattern.append("o")
            elif not state.is_abnormal():
                pattern.append("N")
            else:
                pattern.append("X")  # 其他异常状态
